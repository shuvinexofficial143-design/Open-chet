...
export default function Workspace() {
  ...
  return <div className={`app mvp-app ${mobileChat ? 'mobile-chat' : ''}`}>
    ...
    {dialog ? <Dialog title={{contact: dialog.row ? 'Edit contact' : 'Add contact', 'new-chat': 'Start a conversation', 'send-template': 'Choose an approved template', quick_reply: 'Quick reply', product: 'Product details', import: 'Import contacts', 'clear-chat': 'Clear chat', reset: 'Reset demo workspace', 'whatsapp-account': 'Add WhatsApp number', 'whatsapp-label': 'Edit connection name'}[dialog.type] || dialog.type} onClose={() => setDialog(null)}>
      {dialog.type === 'whatsapp-account' ? <form onSubmit={async (event) => {event.preventDefault(); const values = inputValues(event.currentTarget); const next = await connectionAction({action: 'add', ...values}, 'WhatsApp number connected'); if (next) setDialog(null);}}><p className="credential-note">The access token is verified with Meta, encrypted on the server, and never shown again.</p>{field('Connection name / label', 'label', '', 'text', true)}{field('Phone Number ID', 'phone_number_id', '', 'text', true)}{field('WhatsApp Business Account ID', 'business_account_id', '', 'text', true)}{field('Access Token', 'access_token', '', 'password', true)}<button className="primary" disabled={busy}>Verify and connect</button></form> : null}
